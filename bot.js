...
client.on('message', msg => {
  ...
  .setDescription(`napim diyince olur gibi :sunglasses:`)
  .setColor("RANDOM")
...
client.on('message', async message => {
  ...
  const command = args.shift().toLowerCase();
  let u = message.mentions.users.first() || message.author;
  if (command === "v!sunucu-kur") {
  if (message.guild.channels.find(channel => channel.name === "Bot Kullanımı")) return message.channel.send(" Bot Paneli Zaten Ayarlanmış.")
  if (!message.member.hasPermission('ADMINISTRATOR'))
  return message.channel.send(" Bu Kodu `Yönetici` Yetkisi Olan Kişi Kullanabilir.");
    message.channel.send(`Bot Bilgi Kanallarının kurulumu başlatılsın mı? başlatılacak ise **evet** yazınız. **Eğer Vento rolünü en üste almassanız hata oluşabilir.**`)
